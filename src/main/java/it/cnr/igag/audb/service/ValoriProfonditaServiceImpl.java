 ...
 */
@Transactional
@Service("valoriProfonditaService")
public class ValoriProfonditaServiceImpl extends AbstractService implements ValoriProfonditaService {

    @Autowired
    protected ValoriProfonditaDao valoriProfonditaDao;
    
    public ValoriProfonditaServiceImpl() {
        super();
    }
    ...
    public Result<ValoriProfondita> find(Integer idProfondita, Integer actionUserId) {
        if (isValidUser(actionUserId)) {
            ...
            ValoriProfondita profondita = valoriProfonditaDao.find(idProfondita);
            return ResultFactory.getSuccessResult(profondita);
    ...
    public Result<List<ValoriProfondita>> findAll(Integer actionUserId) {
        if (isValidUser(actionUserId)) {
            return ResultFactory.getSuccessResult(valoriProfonditaDao.findAll());
        } else {
            return ResultFactory.getFailResult(USER_INVALID);
        }
    }
